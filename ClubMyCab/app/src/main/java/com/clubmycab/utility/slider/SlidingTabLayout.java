package com.clubmycab.utility.slider;
 ...
 * layout.
 * http://stackoverflow.com/questions/32110525/android-slidingtablayout-selected-tab-typeface-bold
 */
public class SlidingTabLayout extends HorizontalScrollView {
	...
	protected TextView createDefaultTabView(Context context) {
		...
		textView.setTypeface(FontTypeface.getTypeface(context, AppConstants.HELVITICA));
		...
		int padding = (int) (TAB_VIEW_PADDING_DIPS * getResources()
				.getDisplayMetrics().density);
		textView.setPadding(5, 0, 0, padding);
		return textView;
	}
	...
	private void populateTabStrip() {
		...
		for (int i = 0; i < adapter.getCount(); i++) {
			...
			tabTitleView.setTypeface(FontTypeface.getTypeface(context,AppConstants.HELVITICA));
			if(i==0){
				tabTitleView.setTextColor(Color.WHITE);
			}else{
				tabTitleView.setTextColor(Color.parseColor("#a0ffffff"));
			}
	...
	private void scrollToTab(int tabIndex, int positionOffset) {
		...
		if (selectedChild != null) {
			...
			View leftOfSelected = mTabStrip.getChildAt(tabIndex-1);
			View rightOfSelected=null;
			if (tabIndex<=tabStripChildCount-1) {
				rightOfSelected = mTabStrip.getChildAt(tabIndex + 1);
			}

			// New Code------------------------------>
			if (selectedChild != null) {
				TextView selectedText = (TextView) selectedChild;
				TextView leftOfSelectedText = (TextView) leftOfSelected;
				TextView rightOfSelectedText = (TextView) rightOfSelected;
				if (tabIndex > 0 && tabIndex < tabStripChildCount - 1) {
					...
					leftOfSelectedText.setTextColor(getResources().getColor(
							R.color.colorDullWhite));
					rightOfSelectedText.setTextColor(getResources().getColor(
							R.color.colorDullWhite));
				...
				} else if (tabIndex == tabStripChildCount - 1) {
					selectedText.setTypeface(Typeface.DEFAULT_BOLD);
					leftOfSelectedText.setTypeface(Typeface.DEFAULT);
					leftOfSelectedText.setTextColor(getResources().getColor(
							R.color.colorDullWhite));
